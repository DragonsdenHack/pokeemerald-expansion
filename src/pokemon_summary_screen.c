...
static EWRAM_DATA struct PokemonSummaryScreenData
{
    ...
    /*0x70*/ struct PokeSummary
    {
        ...
        u8 ailment; // 0x7
        u8 abilityNum; // 0x8
        u16 metLocation; // 0x9 //Xiros: Los valores cambian desde aca
        u8 metLevel; // 0xA
        u8 metGame; // 0xB
        u32 pid; // 0xC
        u32 exp; // 0x10
...
static const u8 sMemoHiddenNatureTextColor[] = _("{COLOR}{01}");
...
static void PrintTextOnWindow2(u8 windowId, const u8 *string, u8 x, u8 y, u8 lineSpacing, u8 colorId)
{
...
{
    u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);
    PrintTextOnWindow2(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ABILITY), gAbilityDescriptionPointers[ability], 0, 17, 0, 0);
}
...
static void BufferLeftColumnStats(void)
{
   ...
   // ConvertIntToDecimalStringN(currentHPString, sMonSummaryScreen->summary.currentHP, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ...
    gNatureStatTable[(sMonSummaryScreen->summary.hiddenNature == HIDDEN_NATURE_NONE) ? sMonSummaryScreen->summary.nature : sMonSummaryScreen->summary.hiddenNature];
    natureMod = gNatureStatTable[(sMonSummaryScreen->summary.hiddenNature == HIDDEN_NATURE_NONE) ? sMonSummaryScreen->summary.nature : sMonSummaryScreen->summary.hiddenNature];
