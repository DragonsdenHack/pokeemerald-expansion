#ifndef GUARD_BATTLE_TRANSITION_H
#define GUARD_BATTLE_TRANSITION_H

void BattleTransition_StartOnField(u8 transitionId);
void BattleTransition_Start(u8 transitionId);
bool8 IsBattleTransitionDone(void);
bool8 FldEff_PokeballTrail(void);
void Task_BattleTransition_Intro(u8 taskId);
void GetBg0TilesDst(u16 **tilemap, u16 **tileset);

extern const struct SpritePalette gSpritePalette_Pokeball;

enum {
    MUGSHOT_SIDNEY,
    MUGSHOT_PHOEBE,
    MUGSHOT_GLACIA,
    MUGSHOT_DRAKE,
    MUGSHOT_CHAMPION,
    MUGSHOT_CONSUL,
    MUGSHOT_SURGE,
    MUGSHOT_BROCK,
    MUGSHOT_PETRA,
    MUGSHOT_MARCIAL,
    MUGSHOT_ERICO,
    MUGSHOT_CANDELA,
    MUGSHOT_NORMAN,
    MUGSHOT_ALANA,
    MUGSHOT_VITO,
    MUGSHOT_GALANO,
    MUGSHOT_MISTY,
    MUGSHOT_ERIKA,
    MUGSHOT_KOGA,
    MUGSHOT_SETIMA,
    MUGSHOT_SETE,
    MUGSHOT_SABRINA,
    MUGSHOT_BLAINE,
    MUGSHOT_PETREL,
    MUGSHOT_EUSINE,
    MUGSHOT_FRANK,
    MUGSHOT_FRANK2,
    MUGSHOT_PROTON,
    MUGSHOT_ATLAS,
    MUGSHOT_LORELEI,
    MUGSHOT_BRUNO,
    MUGSHOT_AGATHA,
    MUGSHOT_LANCE,
    MUGSHOT_MIRTO,
    MUGSHOT_MIGUEL,
    MUGSHOT_PEGASO,
    MUGSHOT_CESAR,
    MUGSHOT_VALERIA,
    MUGSHOT_CROM,
    MUGSHOT_ANIBAL,
    MUGSHOT_JADEN,
    MUGSHOT_FREDO,
    MUGSHOT_SALAZAR,
    MUGSHOT_ZEUS,
    MUGSHOT_GIOVANNI,
    MUGSHOT_OAK,
    MUGSHOT_ROJO,
    MUGSHOT_JONES,
    MUGSHOT_DIOSJONES,
    MUGSHOT_PRESIDENTE,
    MUGSHOT_CAPITAN,
    MUGSHOT_MAGNO,
    MUGSHOT_AQUILES,
    MUGSHOT_MAESTRODUN,
    MUGSHOT_FUJI,
    MUGSHOT_ANDRA2,
    MUGSHOT_ANDRA3,
    MUGSHOT_ANDRA4,
    MUGSHOT_ANDRA5,
    MUGSHOT_ANDRA6,
    MUGSHOT_LIDERSETE,
    MUGSHOT_FORTUNIA,
    MUGSHOT_DESTRA,
    MUGSHOT_VALENTE,
    MUGSHOT_SACHIKO,
    MUGSHOT_DRACON,
    MUGSHOT_AMATISTA,
    MUGSHOT_INVERNA,
    MUGSHOT_RASMUS,
    MUGSHOT_TRISTANA,
    MUGSHOTS_COUNT
};

enum {
    B_TRANSITION_BLUR,
    B_TRANSITION_SWIRL,
    B_TRANSITION_SHUFFLE,
    B_TRANSITION_BIG_POKEBALL,
    B_TRANSITION_POKEBALLS_TRAIL,
    B_TRANSITION_CLOCKWISE_WIPE,
    B_TRANSITION_RIPPLE,
    B_TRANSITION_WAVE,
    B_TRANSITION_SLICE,
    B_TRANSITION_WHITE_BARS_FADE,
    B_TRANSITION_GRID_SQUARES,
    B_TRANSITION_ANGLED_WIPES,
    B_TRANSITION_SIDNEY,
    B_TRANSITION_PHOEBE,
    B_TRANSITION_GLACIA,
    B_TRANSITION_DRAKE,
    B_TRANSITION_CHAMPION,
    B_TRANSITION_CONSUL,
    B_TRANSITION_SURGE,
    B_TRANSITION_BROCK,
    B_TRANSITION_PETRA,
    B_TRANSITION_MARCIAL,
    B_TRANSITION_ERICO,
    B_TRANSITION_CANDELA,
    B_TRANSITION_NORMAN,
    B_TRANSITION_ALANA,
    B_TRANSITION_VITO,
    B_TRANSITION_GALANO,
    B_TRANSITION_MISTY,
    B_TRANSITION_ERIKA,
    B_TRANSITION_KOGA,
    B_TRANSITION_SETIMA,
    B_TRANSITION_SETE,
    B_TRANSITION_SABRINA,
    B_TRANSITION_BLAINE,
    B_TRANSITION_PETREL,
    B_TRANSITION_EUSINE,
    B_TRANSITION_FRANK,
    B_TRANSITION_FRANK2,
    B_TRANSITION_PROTON,
    B_TRANSITION_ATLAS,
    B_TRANSITION_LORELEI,
    B_TRANSITION_BRUNO,
    B_TRANSITION_AGATHA,
    B_TRANSITION_LANCE,
    B_TRANSITION_PEGASO,
    B_TRANSITION_MIRTO,
    B_TRANSITION_MIGUEL,
    B_TRANSITION_CESAR,
    B_TRANSITION_VALERIA,
    B_TRANSITION_CROM,
    B_TRANSITION_ANIBAL,
    B_TRANSITION_JADEN,
    B_TRANSITION_FREDO,
    B_TRANSITION_SALAZAR,
    B_TRANSITION_ZEUS,
    B_TRANSITION_GIOVANNI,
    B_TRANSITION_OAK,
    B_TRANSITION_ROJO,
    B_TRANSITION_JONES,
    B_TRANSITION_DIOSJONES,
    B_TRANSITION_PRESIDENTE,
    B_TRANSITION_MAGNO,
    B_TRANSITION_AQUILES,
    B_TRANSITION_FUJI,
    B_TRANSITION_CAPITAN,
    B_TRANSITION_MAESTRODUN,
    B_TRANSITION_ANDRA2,
    B_TRANSITION_ANDRA3,
    B_TRANSITION_ANDRA4,
    B_TRANSITION_ANDRA5,
    B_TRANSITION_ANDRA6,
    B_TRANSITION_LIDERSETE,
    B_TRANSITION_FORTUNIA,
    B_TRANSITION_DESTRA,
    B_TRANSITION_VALENTE,
    B_TRANSITION_SACHIKO,
    B_TRANSITION_DRACON,
    B_TRANSITION_AMATISTA,
    B_TRANSITION_INVERNA,
    B_TRANSITION_RASMUS,
    B_TRANSITION_TRISTANA,
    B_TRANSITION_AQUA, // Here below added in Emerald
    B_TRANSITION_MAGMA,
    B_TRANSITION_REGICE,
    B_TRANSITION_REGISTEEL,
    B_TRANSITION_REGIROCK,
    B_TRANSITION_KYOGRE,
    B_TRANSITION_GROUDON,
    B_TRANSITION_RAYQUAZA,
	B_TRANSITION_ROCKET,
	B_TRANSITION_DEVON,
    B_TRANSITION_SHRED_SPLIT,
    B_TRANSITION_BLACKHOLE,
    B_TRANSITION_BLACKHOLE_PULSATE,
    B_TRANSITION_RECTANGULAR_SPIRAL,
    B_TRANSITION_FRONTIER_LOGO_WIGGLE,
    B_TRANSITION_FRONTIER_LOGO_WAVE,
    B_TRANSITION_FRONTIER_SQUARES,
    B_TRANSITION_FRONTIER_SQUARES_SCROLL,
    B_TRANSITION_FRONTIER_SQUARES_SPIRAL,
    B_TRANSITION_FRONTIER_CIRCLES_MEET,
    B_TRANSITION_FRONTIER_CIRCLES_CROSS,
    B_TRANSITION_FRONTIER_CIRCLES_ASYMMETRIC_SPIRAL,
    B_TRANSITION_FRONTIER_CIRCLES_SYMMETRIC_SPIRAL,
    B_TRANSITION_FRONTIER_CIRCLES_MEET_IN_SEQ,
    B_TRANSITION_FRONTIER_CIRCLES_CROSS_IN_SEQ,
    B_TRANSITION_FRONTIER_CIRCLES_ASYMMETRIC_SPIRAL_IN_SEQ,
    B_TRANSITION_FRONTIER_CIRCLES_SYMMETRIC_SPIRAL_IN_SEQ,
    B_TRANSITION_COUNT
};

// IDs for GetSpecialBattleTransition
enum {
    B_TRANSITION_GROUP_B_TOWER,
    B_TRANSITION_GROUP_B_DOME = 3,
    B_TRANSITION_GROUP_B_PALACE,
    B_TRANSITION_GROUP_B_ARENA,
    B_TRANSITION_GROUP_B_FACTORY,
    B_TRANSITION_GROUP_B_PIKE,
    B_TRANSITION_GROUP_B_PYRAMID = 10,
    B_TRANSITION_GROUP_TRAINER_HILL,
    B_TRANSITION_GROUP_SECRET_BASE,
    B_TRANSITION_GROUP_E_READER,
};

#endif // GUARD_BATTLE_TRANSITION_H
